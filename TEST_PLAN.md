...
9. Verify successful analysis renders one card per returned code with `code_label`, `quote`, and `rationale` below the document text.
10. Trigger a backend failure (for example missing `ANTHROPIC_API_KEY` in server env) and verify a friendly error is shown in the UI.
11. Call `POST /api/analyze` with another user's `document_id` and verify it returns `403`.
12. Simulate malformed model output and verify `POST /api/analyze` returns `502` with `error`, truncated `raw_text`, and `response_debug` (`content_block_count`, `first_block_type`).
...
14. Verify successful `POST /api/analyze` response shape is:
    ```json
    {
      "codes": [
        {
