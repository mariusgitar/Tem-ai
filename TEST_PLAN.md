...
9. Simulate malformed model output and verify `POST /api/analyze` returns `502` with a safe non-JSON Anthropic error response.
10. Simulate Anthropic output with markdown fences or prose around JSON and verify `/api/analyze` still extracts and parses the first JSON array.
11. Simulate non-JSON Anthropic output and verify `/api/analyze` returns `502` with `{ "error": "Anthropic returned non-JSON output" }` and (optionally) a short preview.
